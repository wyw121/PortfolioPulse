...
impl BlogService {
    ...
    pub async fn create_post(&self, req: CreateBlogPostRequest) -> Result<BlogPost> {
        let id = Uuid::new_v4().to_string();  // 转换为字符串
        ...
        self.get_post_by_id(&id).await?.ok_or_else(|| anyhow::anyhow!("Failed to create post"))  // 传递引用
    }

    // 管理员功能 - 更新文章
    pub async fn update_post(&self, id: &str, req: UpdateBlogPostRequest) -> Result<BlogPost> {  // 接受 &str
    ...
    // 管理员功能 - 删除文章
    pub async fn delete_post(&self, id: &str) -> Result<()> {  // 接受 &str
    ...
    // 私有辅助方法
    async fn get_post_by_id(&self, id: &str) -> Result<Option<BlogPost>> {  // 接受 &str
