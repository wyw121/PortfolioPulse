...
if ($LASTEXITCODE -eq 0) {
    ...
    try {
        if ($RootPassword) {
            $result = Write-Output $testQuery | mysql -u root -p$RootPassword 2>$null
        } elseif ($SkipPasswordPrompt) {
            $result = Write-Output $testQuery | mysql -u root 2>$null
        } else {
            Write-Host '请再次输入密码进行验证...'
            $result = Write-Output $testQuery | mysql -u root -p
        }
